<script setup>
    ...
    onMounted( async () => {
        ...
        .then(response => {
            ...
            listeCines.value = response.records;
            // On vérifie dans la console l'obtention des résultats
            console.log("Liste des cinémas", listeCines);
            ...
            // Parcours des gares
            listeCines.value.forEach( (cine) =>{
                // Récupération des coordonées de la gare
                let position = cine.geometry.coordinates;
                // Nom de la commune
                let libelle = cine.fields.nom;
                // Ajout d'un marqueur
    ...
    <section>
        <div class="container">
            <select class="custom-select" v-model="depSelect" @change="selection(depSelect)">
                ...
                <option v-for="dep in listeDep" :key="dep.code" :value="dep.code">
                    {{dep.code}} - {{dep.nom}}
                </option>
            </select>
            <hr/>
