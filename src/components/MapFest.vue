<script setup>
    ...
    onMounted( async () => {
        ...
        const selection = async (dep) => {
            ...
            +'&q=departement_principal_de_deroulement='+dep
            +'&rows=10000'
        ...
        .then(response => {
            ...
            listeFest.value = response.records;
            // On vérifie dans la console l'obtention des résultats
            console.log("Liste des festivals", listeFest);
            ...
            // Parcours des gares
            listeFest.value.forEach( (fest) =>{
                if(fest.geometry===undefined){
                    let position=[0,0]
                }   else{
                // Récupération des coordonées de la gare
                let position = fest.geometry.coordinates;
                // Nom de la commune
                let libelle = fest.fields.nom_du_festival;
                // Ajout d'un marqueur
